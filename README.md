...
> 已发布到 npm：[`opencode-session-tui`](https://www.npmjs.com/package/opencode-session-tui)。opencode 的插件加载器把 spec 透传给 [`arborist.reify`](https://github.com/sst/opencode/blob/main/packages/core/src/npm.ts)，`npm-package-arg` 支持的所有形态都能用：`opencode-session-tui@<version>`、`github:owner/repo`、`git+https://…`、`git+ssh://…`、`file:…`。
...
{
  "plugin": [
    "opencode-session-tui@0.1.0",
    "opencode-session-tui@latest"
  ]
}
```

### 方式 B：从 GitHub 仓库安装（追最新 main / 私有部署）

```json
{
  "plugin": [
    "github:wh0isroot/opencode-session-tui",
...
# 3. 发 npm（prepublishOnly 会自动跑 typecheck）
npm publish
```

用户端两种钉法：
- npm 版本号：`"opencode-session-tui@0.1.0"`
- git tag：`"github:wh0isroot/opencode-session-tui#v0.1.0"`
